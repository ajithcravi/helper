...
let generateRelationships = () => {
    ...
    config.Concept.TableRelationDetails.forEach(detail => {
    ...
    public class ${detail.To}To${detail.From}Relationship : RelationshipDescription<${detail.To + config.version}, ${detail.From + config.version}>
    {
        public ${detail.To}To${detail.From}Relationship() : base("${detail.To.toLowerCase()}", "${detail.From.toLowerCase()}")
        {
        }

        protected override IEnumerable<KeyPair> GetKeysOverride(DBRecordBase record)
        {
            var ${detail.To.toLowerCase()} = (${detail.To + config.version})record;
            KeyPair key = new("record.${detail.FromField.toLowerCase()}", ${detail.To.toLowerCase()}.Record.${detail.ToField});
...
let generateRelationshipCalls = () => {
    ...
    let relation = {};
    config.Concept.TableRelationDetails.forEach(detail => {
        if (!relation[detail.From]) relation[detail.From] = [];
        relation[detail.From].push(`new ${detail.From}To${detail.To}Relationship()`)
    })
...
let generateTableRelation = () => {
    try {
        let filePath = config.BaseLocation + config.FileLocation.TableRelations + config.Concept.ConceptName + "ResourceTables" + config.FileFormats.cs;
